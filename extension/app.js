...
function ActivityLog(props) {
  return e('div', { className: 'log' }, [
    props.logs.map((l, i) =>
      e('div', { key: i, className: 'log-entry' }, `${l.time}: ${l.preview}`)
    ),
...
async function startAutoLike(settings, addLog) {
  ...
  while (running && liked < sessionLimit) {
    ...
    if (Date.now() - startTime > 3600000 || liked >= hourLimit) {
      running = false;
    }
  }
  running = false;
...
export function AutoLike() {
  ...
  const start = () => {
    ...
    setIsRunning(true);
    startAutoLike(
      { keywords: keywords.split(/,\s*/), sessionLimit, hourLimit, safeMode },
      addLog
  ...
  return e('div', { className: 'tool-panel' }, [
    ...
    e('div', { key: 's', className: 'field' }, [
      ...
      e('input', {
        type: 'number',
        value: sessionLimit,
        onChange: ev => setSessionLimit(parseInt(ev.target.value) || 0)
      })
    ]),
    e('div', { key: 'h', className: 'field' }, [
      ...
      e('input', {
        type: 'number',
        value: hourLimit,
        onChange: ev => setHourLimit(parseInt(ev.target.value) || 0)
      })
    ]),
  ...
  return e(
    ...
    tools.map(t =>
      e(
        ...
        {
          key: t,
          className: 'side-item' + (t === props.active ? ' active' : ''),
